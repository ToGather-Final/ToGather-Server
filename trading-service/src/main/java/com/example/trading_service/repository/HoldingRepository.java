...
public interface HoldingRepository extends JpaRepository<Holding, UUID> {
    ...
    // 투자 계좌별 보유 종목 조회
    @Query("SELECT h FROM Holding h WHERE h.investmentAccount.investmentAccountId = :accountId")
    List<Holding> findByAccountId(@Param("accountId") UUID accountId);
    
    // 특정 계좌의 특정 종목 보유 조회
    @Query("SELECT h FROM Holding h WHERE h.investmentAccount.investmentAccountId = :accountId AND h.stock.id = :stockId")
    Optional<Holding> findByAccountIdAndStockId(@Param("accountId") UUID accountId, @Param("stockId") UUID stockId);
