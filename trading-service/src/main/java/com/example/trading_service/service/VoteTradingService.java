    ...
    private void checkVoteTradingExecutionPossibility(VoteTradingRequest request, String orderType) {
        try {
            ...
            Stock stock = stockRepository.findById(request.getStockId())
                    .orElseThrow(() -> new IllegalArgumentException("주식을 찾을 수 없습니다: " + request.getStockId()));
            ...
            float requestPrice = request.getPrice().floatValue();
            ...
            if ("BUY".equals(orderType)) {
                ...
                log.info("🗳️ 투표 기반 매수 체결 가능 여부 - 투표ID: {}, 종목코드: {}, 요청가격: {}, 최저매도가: {}, 체결가능: {}", 
                        request.getVoteId(), stock.getStockCode(), requestPrice, lowestAskPrice, canExecute);
                
                if (!canExecute) {
                    log.warn("⚠️ 투표 기반 매수 주문 - 현재 호가에서 체결 불가능: 투표ID={}, 종목코드={}, 요청가격={}", 
                            request.getVoteId(), stock.getStockCode(), requestPrice);
                }
                ...
                log.info("🗳️ 투표 기반 매도 체결 가능 여부 - 투표ID: {}, 종목코드: {}, 요청가격: {}, 최고매수가: {}, 체결가능: {}", 
                        request.getVoteId(), stock.getStockCode(), requestPrice, highestBidPrice, canExecute);
            ...
            log.error("❌ 투표 기반 거래 체결 가능 여부 확인 중 오류 발생 - 투표ID: {} - {}", 
                    request.getVoteId(), e.getMessage());
        }
    }
}
