...
class HistoryServiceTest {
    ...
    void createVoteCreatedHistory_Success() throws Exception {
        ...
        // When
        historyService.createVoteCreatedHistory(userId, proposalId, proposalName, proposerName);

        // Then
        verify(groupMembersRepository).findFirstGroupIdByUserId(userId);
        verify(objectMapper).writeValueAsString(any()); // 1번만 호출
        verify(historyRepository).save(any(History.class)); // 1번만 저장
    }
    ...
    @Test
    @DisplayName("사용자가 그룹에 속하지 않은 경우 - 예외 발생")
    void createVoteCreatedHistory_NoGroup_ThrowsException() {
        // Given
        UUID userId = UUID.randomUUID();
        UUID proposalId = UUID.randomUUID();
        ...
        try {
            ...
            assertThat(e.getMessage()).contains("사용자가 속한 그룹이 없습니다.");
        }
